...
from ..core.water_forecast import Change as WaterForecastChange
from ..core.water_forecast import directional_change, forecast_metric
...
def create_app(application: Application) -> FastAPI:
    ...
    WATER_FORECAST_KEY = "water.forecast"
    WATER_FORECAST_IDS = (
    ...
    def _forecast_metric_json(metric: Any) -> dict[str, Any]:
        return {
            "ready": metric.ready,
            "rate_l_day": metric.rate_l_day,
            "remaining_days": metric.remaining_days,
            "observed_hours": metric.observed_hours,
            "change_l": metric.change_l,
        }

    def _empty_forecast_metric() -> dict[str, Any]:
        return {
            "ready": False,
            "rate_l_day": None,
            "remaining_days": None,
    ...
    def _forecast_unavailable() -> dict[str, Any]:
        return {
            "available": False,
            "started_at": None,
            "fresh": _empty_forecast_metric(),
            "grey": _empty_forecast_metric(),
            "black": _empty_forecast_metric(),
        }

    def _water_now() -> tuple[Any, dict[str, TankView]]:
    ...
    async def _water_forecast_payload(
        ...
        summary, tanks = _water_now()

        changes: dict[str, WaterForecastChange] = {}

        for entity_id in WATER_FORECAST_IDS:
            tank = tanks.get(entity_id)

            if tank is None:
                changes[entity_id] = WaterForecastChange()
                continue
            ...
            changes[entity_id] = directional_change(
                started_at=checkpoint.at,
                now_ms=now_ms,
                start_percent=checkpoint.snapshot.get(
                    entity_id
                ),
                current_percent=tank.percent,
        ...
        fresh = forecast_metric(
            [
                changes["water.tank.fresh.large"],
                changes["water.tank.fresh.small"],
            ],
            remaining_l=summary.fresh.litres,
        )

        grey_tank = tanks.get("water.tank.grey")
        black_tank = tanks.get("water.tank.black")

        grey = forecast_metric(
            [changes["water.tank.grey"]],
            remaining_l=(
                grey_tank.free_l
                if grey_tank is not None
                else None
            ),
        )

        black = forecast_metric(
            [changes["water.tank.black"]],
            remaining_l=(
                black_tank.free_l
                if black_tank is not None
                else None
            ),
        )
        ...
        return {
            "available": True,
            "started_at": checkpoint.at,
            "fresh": _forecast_metric_json(fresh),
            "grey": _forecast_metric_json(grey),
            "black": _forecast_metric_json(black),
        }
    ...
    async def water_forecast() -> dict[str, Any]:
        """Verbrauch und geschätzte Wasserreichweite seit dem letzten Reset."""
        return await _water_forecast_payload()

    @router.post("/water/forecast/reset")
    async def reset_water_forecast() -> dict[str, Any]:
        """Beginnt die Verbrauchsberechnung ab dem aktuellen Stand neu."""
        ...
        if store is None or store.degraded:
            raise HTTPException(
                status_code=503,
                detail="Historie nicht verfügbar",
            )
        ...
        snapshot = {
            entity_id: (
                tanks[entity_id].percent
                if entity_id in tanks
                else None
            )
            for entity_id in WATER_FORECAST_IDS
        }
        ...
        try:
            checkpoint = await store.set_checkpoint(
                WATER_FORECAST_KEY,
                snapshot,
            )
